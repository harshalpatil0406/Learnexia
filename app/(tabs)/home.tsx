...
const CourseItem = React.memo(({ item, onPress, onBookmark, isDark }: { 
  item: Course; 
  onPress: () => void; 
  onBookmark: () => void;
  isDark: boolean;
...
export default function Home() {
  ...
  const ListEmptyComponent = useMemo(() => (
    <View className="items-center justify-center py-20">
      ...
      <Text className={`${isDark ? 'text-gray-400' : 'text-gray-500'} text-lg mt-4`}>No courses found</Text>
  ...
  return (
    <View className={`flex-1 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <View className={`${isDark ? 'bg-gray-800' : 'bg-blue-500'} pt-16 pb-6 px-6`}>
        ...
        {/* Search Bar */}
        <View className={`flex-row items-center ${isDark ? 'bg-gray-700' : 'bg-white'} rounded-2xl px-4 py-3`}>
